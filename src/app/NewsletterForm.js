...
export default function NewsletterForm() {
  ...
  return (
    <div className="mt-12 max-w-md mx-auto bg-slate-800/30 p-6 rounded-2xl border border-slate-800">
      <h3 className="text-lg font-semibold mb-3 text-center">Jetzt exklusiven Beta-Zugang sichern</h3>
      <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-3">
        ...
        />
        <button type="submit" className="bg-blue-600 hover:bg-blue-500 text-white px-6 py-3 rounded-xl font-medium text-sm transition">
          Beitreten
        </button>
      </form>
      {status && <p className="text-xs text-slate-400 mt-3 text-center">{status}</p>}
    </div>
  );
}
